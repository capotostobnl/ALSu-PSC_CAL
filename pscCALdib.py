...
for j in range(len(chan)): # loop through channels
    ...
    s_scale_factor = Rb[j]*10.0 # V/A
    p_scale_factor = Ndcct/s_scale_factor # power supply scale factor A/V
    #p_scale_factor=1.0
    ...
    #sp1 = float(int(10*p_scale_factor*0.9)) #  must be close to current setting to keep error from saturating
    ...
    caput(psc+chan[j]+':SF:DAC_DCCTs-SP', p_scale_factor)
