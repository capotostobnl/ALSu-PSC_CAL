...
Configure LAB_INDEX at top to substitute every instance of 'lab{3}' -> 'lab{<LAB_INDEX>}'.
"""
...
# ----------------- CONFIGURATION -----------------
# Change this to the lab number you want (1,2,3,...). This will replace 'lab{3}' tokens.
LAB_INDEX = 3
SN = 'SN0013'

# File outputs
TIMESTAMP = datetime.now().strftime("%Y%m%d_%H%M%S")
HDF5_FILENAME = f"epics_test_{SN}_{LAB_INDEX}_{TIMESTAMP}.h5"
REPORT_FILENAME = f"epics_test_report_{SN}_{LAB_INDEX}_{TIMESTAMP}.pdf"
PLOTS_DIR = f"plots_{SN}_{LAB_INDEX}_{TIMESTAMP}"
...
def lab_replace(pv_template):
    """Replace literal 'lab{3}' in the template with 'lab{N}' where N is LAB_INDEX."""
    return pv_template.replace('lab{3}', f'lab{{{LAB_INDEX}}}')
...
TABLE_PV_TEMPLATES = [
    "lab{3}NumChannels-Mode",
    "lab{3}Resolution-Mode",
    "lab{3}Bandwidth-Mode",
    "lab{3}Polarity-Mode",
]

# Section 1 DAC set/get PV templates
DAC_SET_PV_TEMPLATES = [
    "lab{3}Chan1:DAC_SetPt-SP",
    "lab{3}Chan2:DAC_SetPt-SP",
    "lab{3}Chan3:DAC_SetPt-SP",
    "lab{3}Chan4:DAC_SetPt-SP",
]
...
# EVR PV
EVR_PV_TEMPLATE = "lab{3}TS-S-I"

# Section 3 control PVs templates
DIGOUT_ON2_TEMPLATES = [f"lab{{3}}Chan{ch}:DigOut_ON2-SP" for ch in [1,2,3,4]]
DIGOUT_PARK_TEMPLATES = [f"lab{{3}}Chan{ch}:DigOut_Park-SP" for ch in [1,2,3,4]]
DIGOUT_ON1_TEMPLATES = [f"lab{{3}}Chan{ch}:DigOut_ON1-SP" for ch in [1,2,3,4]]
SS_TRIG_TEMPLATE1 = "lab{3}Chan1:SS:Trig:Usr"  # snapshot trigger 
SS_TRIG_TEMPLATE2 = "lab{3}Chan2:SS:Trig:Usr"  # snapshot trigger 
SS_TRIG_TEMPLATE3 = "lab{3}Chan3:SS:Trig:Usr"  # snapshot trigger 
SS_TRIG_TEMPLATE4 = "lab{3}Chan4:SS:Trig:Usr"  # snapshot trigger 

# FOFB related
FOFB_IP_PV_TEMPLATE = "lab{3}FOFB:IPaddr-SP"
FOFB_DAISY_PV = "lab{2}FOFB:PacketsRcvd-I"
FOFB_FASTADDR_TEMPLATES = [
    "lab{3}Chan1:FOFB:FastAddr-SP",
    "lab{3}Chan2:FOFB:FastAddr-SP",
    "lab{3}Chan3:FOFB:FastAddr-SP",
    "lab{3}Chan4:FOFB:FastAddr-SP",
]
...
def main():
    ...
    header_para = Paragraph(f"<b>EPICS Test Report</b><br/>{SN} lab{{{LAB_INDEX}}}<br/>{now_str}", styles['Title'])
    ...
    for tmpl in TABLE_PV_TEMPLATES:
        ...
        try:
            ...
            try:
                ...
                passsafe_caput(SS_TRIG_TEMPLATE1,1)
    ...
    for ch in channels:
        pvset = lab_replace(f"lab{{3}}Chan{ch}:DAC_SetPt-SP")
        print(pvset)
    ...
    for ch in channels:
        readpv = lab_replace(f"lab{{3}}Chan{ch}:DAC-I")
    ...
    safe_caput("lab{3}Chan1:SF:AmpsperSec-SP",10)
    safe_caput("lab{3}Chan2:SF:AmpsperSec-SP",10)
    safe_caput("lab{3}Chan3:SF:AmpsperSec-SP",10)
    safe_caput("lab{3}Chan4:SF:AmpsperSec-SP",10)


    # Initialize - set DACs to zero
    print("Initializing: setting DACs to zero and preparing outputs...")
    for ch in channels:
        safe_caput(lab_replace(f"lab{{3}}Chan{ch}:DAC_SetPt-SP"), 0.0)
    # Set Enable High
    ...
    # Set to jump mode (opmode = 0)
    for ch in channels:
        safe_caput(lab_replace(f"lab{{3}}Chan{ch}:DAC_OpMode-SP"), 0)
    time.sleep(5)
        
    # Set to smooth ramp first to 10 Amps
    for ch in channels:
        safe_caput(lab_replace(f"lab{{3}}Chan{ch}:DAC_SetPt-SP"), 10.0)
    time.sleep(5)
    
    # Set to jump mode (opmode = 3)
    for ch in channels:
        safe_caput(lab_replace(f"lab{{3}}Chan{ch}:DAC_OpMode-SP"), 3)
    time.sleep(11)
    # Caput 5.1 into setpoints
    for ch in channels:
        safe_caput(lab_replace(f"lab{{3}}Chan{ch}:DAC_SetPt-SP"), 10.05)
    ...
    # Ramp to 23.9 Amps
    for ch in channels:
        safe_caput(lab_replace(f"lab{{3}}Chan{ch}:DAC_SetPt-SP"), 23.89)
    time.sleep(1)
    ...
    # Ramp back to 0 Amps
    for ch in channels:
        safe_caput(lab_replace(f"lab{{3}}Chan{ch}:DAC_SetPt-SP"), 11.25)
    time.sleep(1)
                
    # ---------------- FOFB Daisy Packet monotonic test ----------------
    # Set DAC_OpMode-SP to 2 per script
    for ch in channels:
        safe_caput(lab_replace(f"lab{{2}}Chan{ch}:DAC_OpMode-SP"), 2)
    time.sleep(5)
    
    report_elements.append(Paragraph("<b>Section 2: FOFB Packet test</b>", styles['Heading2']))
    fofb_pv = lab_replace(FOFB_DAISY_PV)
    print(f"Running FOFB Packet test on {fofb_pv} for {EVR_MONOTONIC_SECONDS}s...")
    samples = []
    start_t = time.time()
    while time.time() - start_t < EVR_MONOTONIC_SECONDS:
        v = safe_caget(fofb_pv)
        try:
            samples.append(float(v))
        except Exception:
            samples.append(None)
    ...
    # Filter None
    samples_filtered = [s for s in samples if s is not None]
    fofb_pass = monotonic_increasing(samples_filtered)
    fofb_status = "PASS" if fofb_pass else "FAIL"
    report_elements.append(Paragraph(f"FOFB PV: {fofb_pv} monotonic test -> <b>{fofb_status}</b>", styles['Normal']))
    print(f"FOFB Daisy Test {fofb_status}")

    # Save FOFB samples to hdf5
    try:
        h5.create_dataset("fofb/samples", data=np.array([np.nan if s is None else s for s in samples]))
    except Exception as e:
        print("HDF5 write evr failed:", e)

    # Plot EVR sequence
    fofb_plot = os.path.join(PLOTS_DIR, "fofb_sequence.png")
    if len(samples_filtered) > 0:
        plot_and_save(samples_filtered, f"FOFB {fofb_pv}", fofb_plot)
        report_elements.append(Image(fofb_plot, width=400, height=150))
        report_elements.append(Spacer(1,12))

    # ---- FOFB test if Bandwidth-Mode is "Fast" ----        
    #bandpv = lab_replace("lab{3}Bandwidth-Mode")
    bandval = caget('lab{3}Bandwidth-Mode',as_string=True)
    if bandval is not None and str(bandval).strip().lower() == "fast":
        ...
        # Caput IPaddr
        safe_caput(lab_replace(FOFB_IP_PV_TEMPLATE), int(0x0a451a37))
        #Set IP for Daisy Chain to adjacent PSC
        safe_caput(lab_replace('lab{2}FOFB:IPaddr-SP'), int(0x0a451a37))
        ...
        # Set DAC_OpMode-SP to 2 per script
        for ch in channels:
            safe_caput(lab_replace(f"lab{{3}}Chan{ch}:DAC_OpMode-SP"), 2)
        time.sleep(5)

        # Check that DAC-I etc are ~1.0
        ofc_table = [["PV", "Value", "Pass?"]]
        ofc_pass_all = True
        check_pvs = [f"lab{{3}}Chan{ch}:DAC-I" for ch in channels]
        for tmpl in check_pvs:
            ...
            try:
                f = float(val)
                status = "PASS" if abs(f - 11.5) <= DAC_TOLERANCE else "FAIL"
            except Exception:
