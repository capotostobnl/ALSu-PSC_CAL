...
Configure LAB_INDEX at top to substitute every instance of 'lab{2}' -> 'lab{<LAB_INDEX>}'.
"""
...
# ----------------- CONFIGURATION -----------------
# Change this to the lab number you want (1,2,3,...). This will replace 'lab{2}' tokens.
LAB_INDEX = 2
SN = 'SN0025'
...
def lab_replace(pv_template):
    """Replace literal 'lab{2}' in the template with 'lab{N}' where N is LAB_INDEX."""
    return pv_template.replace('lab{2}', f'lab{{{LAB_INDEX}}}')
...
TABLE_PV_TEMPLATES = [
    "lab{2}NumChannels-Mode",
    "lab{2}Resolution-Mode",
    "lab{2}Bandwidth-Mode",
    "lab{2}Polarity-Mode",
]

# Section 1 DAC set/get PV templates
DAC_SET_PV_TEMPLATES = [
    "lab{2}Chan1:DAC_SetPt-SP",
    "lab{2}Chan2:DAC_SetPt-SP",
    "lab{2}Chan3:DAC_SetPt-SP",
    "lab{2}Chan4:DAC_SetPt-SP",
]
...
# EVR PV
EVR_PV_TEMPLATE = "lab{2}TS-S-I"

# Section 3 control PVs templates
DIGOUT_ON2_TEMPLATES = [f"lab{{2}}Chan{ch}:DigOut_ON2-SP" for ch in [1,2,3,4]]
DIGOUT_PARK_TEMPLATES = [f"lab{{2}}Chan{ch}:DigOut_Park-SP" for ch in [1,2,3,4]]
DIGOUT_ON1_TEMPLATES = [f"lab{{2}}Chan{ch}:DigOut_ON1-SP" for ch in [1,2,3,4]]
SS_TRIG_TEMPLATE1 = "lab{2}Chan1:SS:Trig:Usr"  # snapshot trigger 
SS_TRIG_TEMPLATE2 = "lab{2}Chan2:SS:Trig:Usr"  # snapshot trigger 
SS_TRIG_TEMPLATE3 = "lab{2}Chan3:SS:Trig:Usr"  # snapshot trigger 
SS_TRIG_TEMPLATE4 = "lab{2}Chan4:SS:Trig:Usr"  # snapshot trigger 

# FOFB related
FOFB_IP_PV_TEMPLATE = "lab{2}FOFB:IPaddr-SP"
FOFB_DAISY_PV = "lab{2}FOFB:PacketsRcvd-I"
FOFB_FASTADDR_TEMPLATES = [
    "lab{2}Chan1:FOFB:FastAddr-SP",
    "lab{2}Chan2:FOFB:FastAddr-SP",
    "lab{2}Chan3:FOFB:FastAddr-SP",
    "lab{2}Chan4:FOFB:FastAddr-SP",
]
...
def main():
    ...
    for ch in channels:
        pvset = lab_replace(f"lab{{2}}Chan{ch}:DAC_SetPt-SP")
        print(pvset)
    ...
    for ch in channels:
        readpv = lab_replace(f"lab{{2}}Chan{ch}:DAC-I")
    ...
    safe_caput("lab{2}Chan1:SF:AmpsperSec-SP",10)
    safe_caput("lab{2}Chan2:SF:AmpsperSec-SP",10)
    safe_caput("lab{2}Chan3:SF:AmpsperSec-SP",10)
    safe_caput("lab{2}Chan4:SF:AmpsperSec-SP",10)


    # Initialize - set DACs to zero
    print("Initializing: setting DACs to zero and preparing outputs...")
    for ch in channels:
        safe_caput(lab_replace(f"lab{{2}}Chan{ch}:DAC_SetPt-SP"), 0.0)
    # Set Enable High
    ...
    # Caput 10.00
    for ch in channels:
        safe_caput(lab_replace(f"lab{{2}}Chan{ch}:DAC_SetPt-SP"), 10.0)
    # Set to jump mode (opmode = 3 
    for ch in channels:
        safe_caput(lab_replace(f"lab{{2}}Chan{ch}:DAC_OpMode-SP"), 3)
    time.sleep(11)
    # Caput 10.05 into setpoints
    for ch in channels:
        safe_caput(lab_replace(f"lab{{2}}Chan{ch}:DAC_SetPt-SP"), 10.05)
    ...
    for ch in channels:
        safe_caput(lab_replace(f"lab{{2}}Chan{ch}:DAC_OpMode-SP"), 0)
    time.sleep(1)
    ...
    # Ramp to -11.9 Amps
    for ch in channels:
        safe_caput(lab_replace(f"lab{{2}}Chan{ch}:DAC_SetPt-SP"), 11.9)
    time.sleep(1)
    ...
    #bandpv = lab_replace("lab{2}Bandwidth-Mode")
    bandval = caget('lab{2}Bandwidth-Mode',as_string=True)
    if bandval is not None and str(bandval).strip().lower() == "fast":
        ...
        ofc_pass_all = True
        check_pvs = [f"lab{{2}}Chan{ch}:DAC-I" for ch in channels]
